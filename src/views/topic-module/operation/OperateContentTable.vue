<template>
  <div class="operate-con-table mt-20 ml-10">
    <el-form :inline="true" label-width="100px" :model="search">
      ...
      <el-form-item label="">
        <el-input v-model="search.name" @keyup.native.enter="changeSize(pageSize)"
          placeholder="请输入内容名称..." clearable size="small"></el-input>
      </el-form-item>

      <el-form-item label="">
        <el-input class="input-pos" v-model="resource_name" readonly
          @click.native="showModal('operatePosSelectModal', 'searchbar')" suffix-icon="close"
          placeholder="请输入运营位ID..." clearable size="small"></el-input>
      </el-form-item>
    ...
    <el-table :data="dataList" :default-sort="{prop: 'id', order: 'descending'}"
      :row-class-name="getRowClassName" :header-cell-style="{background:'#FAF9F7',color:'#000000'}"
      ...
      <el-table-column align="center" width="140px;" label="内容名称" prop="name">
        <template slot-scope="scope">
          <a href="javascript:;" class="el-button--text"
            @click.stop="showEditModal(scope.row)">{{scope.row.name}}</a>
        </template>
      </el-table-column>

      <el-table-column align="center" label="内容" width="200">
        <template slot-scope="scope">
          <img v-if="scope.row.resource_type == 1 || scope.row.resource_type == 3"
            class="operate-con-img" :src="scope.row.image_url" v-show="scope.row.image_url" />
          <p v-else class="text-content" v-html="getContentByType(scope.row)"></p>
        </template>
      </el-table-column>
      <el-table-column align="center" label="排序" prop="weight" width="100"
      ...
      <el-table-column align="center" label="操作" width="250">
        <template slot-scope="scope">
          ...
          <el-link class="ml-10" type="primary"
            @click.stop="setStatusForRow(scope.row, scope.row.status)">
    ...
    <operate-pos-select-modal ref="operatePosSelectModal"
      :select-pos-for="selectPosFor"></operate-pos-select-modal>
