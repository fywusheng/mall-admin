<template>
  <div class="main-content">
    <el-form :inline="true" label-width="100px" :model="searchParam">
      <el-form-item label="">
        <el-input v-model="searchParam.page_id" @keyup.native.enter="changeSize(pageSize)"
          placeholder="请输入专题页面标识..." clearable size="small"></el-input>
      </el-form-item>
      <el-form-item label="">
        <el-input v-model="searchParam.page_name" @keyup.native.enter="changeSize(pageSize)"
          placeholder="请输入专题页面名称..." clearable size="small"></el-input>
      </el-form-item>
    ...
    <el-table :data="dataList" :header-cell-style="{background:'#FAF9F7',color:'#000000'}"
      size="mini" v-loading="loading">
      <div slot="empty" class="empty-wrap"><i class="iconfont icon-tishi"></i><span>尚未添加专题</span>
      </div>
      <el-table-column label="专题标识" prop="page_id" width="200px">
      </el-table-column>
      <el-table-column prop="page_name" label="专题名称">
      </el-table-column>
      <el-table-column prop="url" label="专题链接" width="350px">
        <template slot-scope="scope">
          <el-button type="text" size="small"
            @click="online(scope.row)">{{ scope.row.url }}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="category" label="类型" width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column label="发布人" width="100px">
        <template slot-scope="scope">
      ...
      <el-table-column width="220px;" label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-link class="ml-10" type="primary"
            @click="showModal(scope.row, 'modal', true)">复制</el-link>
    ...
    <el-pagination background v-show="totalSize > 10" :page-size="pageSize" :current-page="pageFrom"
      @size-change="changeSize" @current-change="changePage"
      layout="total, slot, jumper, prev, pager, next" :total="totalSize">
    </el-pagination>

    <edit-modal ref="modal" :current="curTopic" :origin="srcTopic" @close="close"
      @save-done="loadData"></edit-modal>
  </div>
</template>
<script>
