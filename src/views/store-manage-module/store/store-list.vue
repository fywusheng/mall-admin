  ...
  <div class="main-content">
    <el-row class="mb-2">
      <el-form :inline="true">
        ...
        <el-form-item label="" prop="infomationNo">
          <franchisee-select
            v-model="searchParams.infomationNo"
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span
            >{{ scope.row.periodStartValidity }} 至
            {{ scope.row.periodEndValidity }}</span
          >
        </template>
      </el-table-column>
      <!-- 这个字段文档中没有 -->
      ...
      >
        <template slot-scope="scope">
          ...
          >
          <el-button
            v-if="scope.row.reviewStatus == 1 && scope.row.yn == 1"
            :disabled="scope.row.id == 1"
            icon="el-icon-turn-off"
            size="mini"
            @click="toggle(scope.row, 0)"
            >停用</el-button
          >
          <el-button
            v-if="scope.row.reviewStatus == 1 && scope.row.yn == 0"
            :disabled="scope.row.id == 1"
            icon="el-icon-open"
            size="mini"
            @click="toggle(scope.row, 1)"
            >启用</el-button
          >
          <el-button
            v-if="scope.row.reviewStatus == 1 && isExpire(scope.row)"
            :disabled="scope.row.id == 1"
            icon="el-icon-set-up"
            size="mini"
...
export default {
  ...
  async mounted() {
    ...
    if (this.$route.query && this.$route.query.type >= 0) {
      this.searchParams.reviewStatus = parseInt(this.$route.query.type);
      this.loadData();
    } else {
      this.loadData();
    }
  ...
  methods: {
    ...
    renewal(row) {
      post("/srm/sh/stores/saveStores", { ...row, renewalStatus: 0 }).then(
        (res) => {
          if (res.code == 200) {
    ...
    formatStatus(row, column) {
      const res = this.examineOptions.find(
        (item) => item.value == row.reviewStatus
      );
