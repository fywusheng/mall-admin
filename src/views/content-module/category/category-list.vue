<template>
  <div class="page-interest app-container">
    <div class="content">
      ...
      <div class="table-wrap">
        ...
        <el-table ref="table" v-loading="listLoading" height="460px" :data="list"
          ...
          <el-table-column label="所属专栏" prop="printName" min-width="120" align="center"
            show-overflow-tooltip />
          <el-table-column label="所属城市" prop="region" min-width="120" align="center"
            show-overflow-tooltip />
          <el-table-column label="文章状态" prop="chkStas" min-width="120" align="center"
            show-overflow-tooltip>
            <template slot-scope="scope">
    ...
    <!-- 审核弹框 -->
    <el-dialog :close-on-click-modal="false" title="审核" :visible.sync="dialogExamine" width="750px"
      class="examine-dialog add-dialog">
      <div class="mobiel flex-c-s flex-col">
        <div class="title">{{detailInfo.ttl}}</div>
        <audio refs="audio" v-if="detailInfo.artiType === '0'" controls="controls"
          :src="detailInfo.mediaUrl">您的浏览器不支持 audio 标签。</audio>
        <video refs="video" v-if="detailInfo.artiType === '1'" style="width: 100%;"
          :src="detailInfo.mediaUrl" controls="controls">您的浏览器不支持 video 标签。</video>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" size="small" @click="sendRightNow(detailInfo.contId)">立即发布
        </el-button>
      </div>
    </el-dialog>

    <!-- 选择日期弹框 -->
    <el-dialog :close-on-click-modal="false" title="定时发布" :visible.sync="dialogTime" width="400px"
      class="time-dialog add-dialog">
      <el-form ref="timeForm" :model="inTimeParams" size="small">
        <el-form-item label="选择定时发布的时间" class="all" prop="releaseDate"
          :rules="[{required: true, message: '请选择定时发布的时间', trigger: 'change' }]">
          <el-date-picker v-model="inTimeParams.releaseDate" format="yyyy-MM-dd HH:mm"
            :picker-options="timePickerOptions" value-format="yyyy-MM-dd HH:mm" type="datetime"
            placeholder="请选择定时发布的时间"></el-date-picker>
          <div class="tips" v-show="inTimeParams.releaseDate">本文章将于 <span
              class="time">{{inTimeParams.releaseDate}}</span> 发布</div>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
    ...
    <!-- 新增或编辑 -->
    <el-dialog :close-on-click-modal="false" :title="addFlag?'新增':flag == 1?'查看':'修改'"
      :visible.sync="dialogFormVisible" width="1250px" class="add-dialog">
      <el-form ref="formAdd" :model="formAdd" :rules="formAddRule" label-width="130px" size="small">
        <el-form-item label="内容类型" class="half" prop="artiType">
          <el-radio v-model="formAdd.artiType" @change="articleImgChange('typeChange')"
            v-for="(item, index) in artiTypeList" :key="index" :label="item.value">{{item.label}}
          </el-radio>
        </el-form-item>
        <el-form-item :label="formAdd.artiType === '0' ? '文章标题' : '视频标题'" class="half" prop="ttl">
          <el-input v-model="formAdd.ttl" clearable />
        </el-form-item>
        <el-row>
          <el-col :span="12">
            <el-form-item label="所属栏目" class="w-100" prop="printId">
              <el-select v-model="formAdd.printId" clearable placeholder="请选择所属栏目"
                @change="handleChangePrintId">
                <el-option v-for="item in categoryTypeOPt" :key="item.value" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item v-if="categoryChildTypeOPt.length>0" label-width="20px" label=""
              ...
              <el-select v-model="formAdd.subColId" clearable placeholder="请选择所属子栏目">
                <el-option v-for="item in categoryChildTypeOPt" :key="item.value"
        ...
        <el-form-item label="是否热点" class="half" prop="recomFlag" v-show="formAdd.artiType === '0'">
          <el-radio v-model="formAdd.recomFlag" v-for="(item, index) in moveFlagList" :key="index"
            :label="item.value">{{item.label}}</el-radio>
        </el-form-item>
        <el-form-item v-show="formAdd.artiType === '0'" label="展示封面" class="upload-img"
          prop="picList">
          <el-radio v-model="formAdd.tempType" @change="tempTypeChange"
            v-for="(item, index) in tempTypeList" :key="index" :label="item.value">{{item.label}}
          </el-radio>
          <y-upload-img ref="articleImg" :imgList="artImgList" accept=".jpg,.jpeg,.png"
            @imgChange="articleImgChange" v-show="formAdd.tempType !== '0'"
            :max="(formAdd.tempType === '1' || formAdd.tempType === '3') ? 1 : 3"></y-upload-img>
          <span class="tips" v-show="formAdd.tempType !== '0'">优质的封面有利于推荐，格式支持JPEG、PNG</span>
        </el-form-item>
        <el-form-item v-show="formAdd.artiType === '1'" label="视频封面" class="upload-img"
          prop="picList">
          <y-upload-img ref="vedioImg" :imgList="videoImgList" :max="1" accept=".jpg,.png"
            @imgChange="articleImgChange"></y-upload-img>
          <span class="tips">优质的封面有利于推荐，格式支持JPEG、PNG</span>
        </el-form-item>
        <el-form-item label="所属城市" class="half" prop="cityCodg">
          <y-cascader ref="yCascaderFormAdd" @change="formAddCityChange" :data="cityList" clearable
            v-model="formAdd.cityCodg" placeholder="请选择所属城市" maxLevel="1" :delChildren="true"
            :settings="{'checkStrictly': false,'level': 'admdvsLv','value':'regnCode','label':'regnName'}" />
        </el-form-item>
        <el-form-item label="内容标签" class="half" size="small" prop="tagNameArr">
          <el-tag :key="tag" v-for="tag in formAdd.tagNameArr" closable type="warning" size="small"
        ...
        <el-form-item label="视频文件" class="half" prop="mediaUrl" v-show="formAdd.artiType === '1'">
          ...
          <div class="mediaUrl">
            <el-radio v-model="hanUrl" label="1">有链接</el-radio>
            <el-input v-show="hanUrl === '1'" class="input" v-model="formAdd.mediaUrl" size="small"
              placeholder="点击上传或输入已地址链接">
            </el-input>
          </div>
          <div class="mediaUrl">
        ...
        <el-form-item label="文章内容" prop="cont"
          :rules="formAdd.artiType === '0' ? [{ required: true, message: '正文内容不能为空', trigger: 'blur'}] : []"
          v-show="formAdd.artiType === '0'">
          <y-editor v-model="formAdd.cont"></y-editor>
        </el-form-item>
      </el-form>
    ...
    <!-- 审核不通过 -->
    <el-dialog class="dialog" title="提示" width="550px" :visible.sync="showDialog">
      <el-form class="data-form" :model="dataForm" ref="dataForm" label-width="150px"
        label-position="top" size="small">
        <div>
          <el-form-item label="是否确认审核不通过？" prop="reason">
            <el-input v-model="dataForm.reason" type="textarea" maxlength="10" :rows="5"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="success" @click="rejectAudit()">
          确定
        </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      formAdd: {
        ...
        printId: "", //栏目id
        subColId: "", //子栏目id
        moveFlag: "0", //置顶标志 （‘0’：非置顶 ‘1’：置顶）
        recomFlag: "0", //热点标志（“0”：非热点，”1”:热点）
      ...
      formAddRule: {
        ttl: [
          ...
          // { required: true, message: "标题长度应该在5-30字之间", trigger: "blur" },
  ...
  methods: {
    ...
    handleBtnPermission() {
      ...
      const btnPermissions = this.getNodeById(menuList, currentPath)
      const res = []
      btnPermissions.children.forEach(item => {
        res.push(item.title)
      })
      this.btnPermissions = res
    },
    // 将tree变成一个数组
    getNodeById(tree, path) {
      // debugger
      const arr = Array.isArray(tree) ? tree : [tree]
      let result = null
      while (arr.length) {
        const item = arr.pop()
        console.log("对比----", path, item.path)
    ...
    // 选择栏目事件
    async handleChangePrintId(value) {
      // console.log(value)
      const params = {
        "printId": value,
        "startTime": "",
      ...
      this.categoryChildTypeOPt = data.list.map((item) => {
        return {
          label: item.colName,
          value: item.colId
        }
 ...
 */
    ...
    // 查看原因
    lookReason({ reason }) {
      this.$alert(`<div style="color:#333333">${reason}</div>`, "查看审核不通过原因", {
 ...
 */
    ...
    async addOrEdit(data = {}, flag) {
      ...
      if (data.contId) {
        ...
        this.formAdd.contId = ""
        this.formAdd.artiType = "0"
        this.formAdd.ttl = ""
        this.formAdd.subTtl = ""
        this.formAdd.printId = ""
        this.formAdd.moveFlag = "0"
        this.formAdd.recomFlag = "0"
        this.formAdd.voiceFlag = "0"
        this.formAdd.tempType = "0"
        this.formAdd.region = "全国"
        this.formAdd.cityCodg = "000000"
        this.formAdd.tagNameArr = []
        this.formAdd.picList = []
        this.formAdd.mediaUrl = ""
        this.formAdd.cont = " "
        this.formAdd.contTypeName = "随便"
      }
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs.formAdd.$el.scrollTo({
          top: 0,
          left: 0,
          behavior: "smooth"
        })
 ...
 */
    getRegnAreaTree() {
      this.selectmenuList = []
      post("/common/cusc/api/usersearch/getRegnAreaTree").then(data => {
