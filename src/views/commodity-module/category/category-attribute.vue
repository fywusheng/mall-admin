    ...
    <el-form ref="dataForm" :model="dataForm" :rules="dataRules">
      ...
      <table width="100%">
        <tr style="height:40px">
          ...
          <td width="60%">
            <el-form-item label="请选择商品SKU规格参数：" prop="specIdsSelected">
              <el-select multiple v-model="dataForm.specIdsSelected"
                style="margin-left:10px;width:300px" @change="specSelectionOnChange"
                :multiple-limit="2" placeholder="请选择商品SKU规格项，必须选2个...">
                <el-option v-for="item in filterSpecsList" :key="item.id" :label="item.name"
        ...
        <tr style="height:100px">
          ...
          <td align="center" colspan="2">
            <el-table ref="spec_mytable" :data="spec_tableData" style="width:100%;font-size:10px">
              ...
              <el-table-column label="规格参数值">
                <div slot-scope="scope">
                  <el-checkbox :indeterminate="scope.row.isIndeterminate"
                    v-model="scope.row.checkAll" @change="checkAllSpec(scope.row)">全选</el-checkbox>
                  <el-checkbox-group @change="checkSpec(scope.row)"
                    v-model="scope.row.attrValSelected">
                    <el-checkbox v-for="attrVal in scope.row.attrValList" :label="attrVal.id"
                      :key="attrVal.id" style="font-size:10px">{{ attrVal.val }}
                    </el-checkbox>
                  </el-checkbox-group>
                </div>
              </el-table-column>
      ...
      <table width="100%">
        <tr style="height:40px">
          ...
          <td width="60%">
            <el-form-item label="请选择商品普通属性参数：" prop="attrIdsSelected">
              <el-select multiple v-model="dataForm.attrIdsSelected"
                style="margin-left:10px;width:300px" @change="attrSelectionOnChange"
                :multiple-limit="10" placeholder="请选择商品普通属性项,最多10个...">
                <el-option v-for="item in filterAttrsList" :key="item.id" :label="item.name"
                  :value="item.id"></el-option>
              </el-select>
            </el-form-item>
          </td>
        ...
        <tr style="height:100px">
          <td width="5%"></td>
          <td align="center" colspan="2">
            <el-table ref="attr_mytable" :data="attr_tableData" style="width:100%;font-size:10px">
              <el-table-column type="index" label="序号"></el-table-column>
              <el-table-column prop="name" label="属性项" width="150px"></el-table-column>
              <el-table-column label="属性参数值">
                <div slot-scope="scope">
                  <el-checkbox :indeterminate="scope.row.isIndeterminate"
                    v-model="scope.row.checkAll" @change="checkAllAttr(scope.row)">全选</el-checkbox>
                  <el-checkbox-group @change="checkAttr(scope.row)"
                    v-model="scope.row.attrValSelected">
                    <el-checkbox v-for="attrVal in scope.row.attrValList" :label="attrVal.id"
                      :key="attrVal.id" style="font-size:10px">{{ attrVal.val }}
                    </el-checkbox>
