<template>
  <el-dialog class="dialog" :title="dataForm.id ? '编辑品牌信息':'创建品牌信息'" width="600px"
    :visible.sync="showDialog">
    <el-form class="data-form" :model="dataForm" :rules="dataRules" ref="dataForm"
      label-width="150px" label-position="top" size="small">
      <el-form-item label="品牌名称(中文)" prop="name">
        <el-input v-model="dataForm.name" placeholder="请输入品牌中文名称..." style="width:100%"
          maxlength="32"></el-input>
      </el-form-item>
      <el-form-item label="品牌名称(英文)" prop="nameEn">
        <el-input v-model="dataForm.nameEn" placeholder="请输入品牌英文名称..." style="width:100%"
          maxlength="32"></el-input>
      </el-form-item>
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" size="mini" :loading="sending" :disabled="sending" @click="save">
        {{sending ?
        '正在保存...':'确定'}}
      </el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleLogoAvatarSuccess(response, file) {
      ...
      this.dataForm.brandLogo = file.response.data.absoluteUrl;
    },
    handleImgAvatarSuccess(response, file) {
      if (response.code != 0) {
        return;
      }
      this.dataForm.brandImage = file.response.data.absoluteUrl;
    },
