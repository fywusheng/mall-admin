...
export default {
  ...
  data() {
    ...
    var validatorSkuPrice = (rule, value, callback) => {
      if (
        this.dataForm.price.sellingPrice.length == 0 ||
        this.dataForm.price.costPrice.length == 0 ||
        this.dataForm.price.markOffPrice.length == 0
      ) {
  ...
  async mounted() {
    if (this.$route.params.id) {
      this.loadCategorySpec(
        this.$route.params.categoryNode,
