  ...
  <div class="div-layout">
    ...
    <el-row>
      <el-col>
        <el-form :inline="true">
          <el-form-item>
            <el-button type="primary" icon="el-icon-search" @click="loadData(productId)"
              size="mini">查询</el-button>
            <el-button type="default" icon="el-icon-circle-plus-outline" @click="add" size="mini"
              :disabled='saleState'>新增</el-button>
          </el-form-item>
        </el-form>
      </el-col>
    </el-row>
    <el-table class="custom-table" :data="specSkuList"
    ...
    <table width="100%">
      ...
      <tr>
        <td>
          <el-table class="custom-table" :data="specImgList"
            :header-cell-style="{ background: '#FAF9F7', color: '#000000' }" size="mini"
            @row-click="selectedRadio" v-loading="loading">
            <div slot="empty" class="empty-wrap">
              <i class="iconfont icon-tishi"></i><span>未查到商品规格图数据，上传</span>
            </div>
            <el-table-column label="选择" align="center" width="50px">
              <template slot-scope="scope">
                <el-radio v-model="selectedRadioValue" :label="scope.row.firstClassAttrId"
                  :disabled='scope.row.saleState==5'></el-radio>
              </template>
            </el-table-column>
            <el-table-column type="index" label="序号" align="center"> </el-table-column>
            <el-table-column prop="firstClassAttrId" label="主规格ID" align="center" width="100px">
            </el-table-column>
            <el-table-column prop="firstClassAttrName" label="主规格名" width="150px">
            </el-table-column>
            <el-table-column prop="" label="规格图片">
              <template slot-scope="scope">
                <el-upload list-type="picture-card" :limit="2" accept="image/*"
                  :file-list="scope.row.specImgUrlList" :before-upload="beforeAvatarUpload"
                  :on-remove="handleRemove" :on-success="handleChange" :on-exceed="exceedFile"
                  :class="{hide:scope.row.hideUploadCard}" :auto-upload="true"
