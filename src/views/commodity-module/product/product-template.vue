  ...
  <div class="div-layout">
    ...
    >
      ...
      <table width="100%">
              ...
              <tr>
                ...
                <td>
                  <div>
                    ...
                    商品主图(按顺序上传，最多上传5张图片，每张尺寸800*800px，大小1MB以内，图片格式jpg、png)
                  </div>
                  <el-form-item prop="mainImgUrl">
                    ...
                    >
                      <el-upload
                        :disabled="
                          dataForm.saleState == 5 || uploadImgIndex < index
                        "
                  ...
                  <div>
                    <span style="color: #ff4949">*</span>
                    商品视频(时长不超过60秒，尺寸为16:9，分辨率1920x1080或1280x720，格式MP4或FLV，大小500MB以内)
                  </div>
                  <el-upload
                    class="upload-demo"
        ...
        <tr>
          ...
          <td width="45%">
            <el-form-item
              label="前台类目"
              prop="firstCategoryNode"
              class="item _custom_cascader"
            >
              <el-cascader
                v-model="dataForm.firstCategoryNode"
                :options="frontDeskCategoriesOptions"
                placeholder="请选择商品所属类目信息..."
                :disabled="dataForm.saleState == 5"
                :props="{
                  value: 'id',
          ...
          <td width="50%">
            <el-form-item label="计量单位" prop="valuationUnit" class="item">
              <el-select
                v-model="dataForm.valuationUnit"
        ...
        <tr>
          ...
          <td width="45%">
            <el-form-item label="适用人群" prop="targetAudience" class="item">
              <el-select
                v-model="dataForm.targetAudience"
                multiple
                :multiple-limit="2"
                clearable
                placeholder="请选择最多不超过2个适用人群标签..."
                :disabled="dataForm.saleState == 5"
                style="width: 80%"
              >
                <el-option
      ...
      <table width="100%" v-if="dataForm.saleState == 5">
        <tr>
          <td width="5%"></td>
          <td width="45%">
            <el-form-item label="销售状态" prop="saleState" class="item"
              >销售中(销售中商品不可编辑)</el-form-item
            >
          </td>
          <td width="50%">
            <el-form-item label="数据状态" prop="delFlag" class="item"
              >正常</el-form-item
            >
          </td>
        </tr>
    ...
    >
      <el-form-item label="是否支持会员优惠:" prop="memberDiscount">
        ...
        <el-input
          v-if="oldMoneyForm.memberDiscount == 1"
          v-model="oldMoneyForm.discountAmount"
          maxlength="5"
      ...
      <el-form-item label="是否支持积分抵扣:" prop="isCreditPoints">
        ...
        <template v-if="oldMoneyForm.isCreditPoints == 1">
          ...
          <el-input
            v-model="oldMoneyForm.pointDiscountPoint"
            maxlength="5"
            @blur="limitFiveStr"
        ...
        <template v-if="oldMoneyForm.isCreditPoints == 1">
          ...
          <el-input
            v-model="oldMoneyForm.registerPoint"
            maxlength="5"
            @blur="limitFiveStr"
    ...
    >
      ...
      >
        <el-cascader
          ref="provinceSP"
          class="autoProvice"
          v-model="shopForm.selfSelectArray"
          placeholder="请选择地区"
          :options="areaList"
          :props="{
            multiple: true,
      ...
      <el-form-item label="发货地类型:" prop="deliveryType" class="item">
        <el-select
          v-model="shopForm.deliveryType"
          style="width: 80%"
          placeholder="请选择商品发货地类型..."
          :disabled="dataForm.saleState == 5"
          maxlength="100"
        >
          <el-option
      ...
      <el-form-item label="发货地:" prop="deliveryRegion" class="item">
        <el-input
          v-model="shopForm.deliveryRegion"
      ...
      <el-form-item
        label="发货时长(N1至N2天送达):"
        prop="deliveryMinDays"
        class="item"
      >
        <div class="lineFlex">
          <el-input-number
            v-model="shopForm.deliveryMinDays"
          ...
          -至-
          <el-input-number
            v-model="shopForm.deliveryMaxDays"
            controls-position="right"
    ...
    >
      ...
      <el-form-item
        v-if="serviceForm.store == '0' && serviceForm.place == '1'"
        label="请选择地区:"
        prop="selfSelectArray"
      >
        <el-cascader
          ref="proviceFW"
          class="autoProvice"
          v-model="serviceForm.selfSelectArray"
          placeholder="请选择地区"
          :options="areaList"
          :props="{
            multiple: true,
    ...
    <el-dialog title="选择门店" :visible.sync="dialogTableVisible">
      <el-cascader
        ref="tkProvince"
        v-if="serviceForm.place == '1'"
        class="autoProvice"
        v-model="tkPriveArray"
        placeholder="请选择地区"
        :options="areaList"
        :props="{
          multiple: false,
...
export default {
  ...
  data() {
    ...
    return {
      ...
      dataRules: {
        ...
        attributeMap: [
          { required: true, trigger: "blur", validator: validateParam },
        ],
      },
      uploading: false,
      editorOptions: {
  ...
  methods: {
    ...
    async loadSupplierOptions() {
      const result = await post("/srm/supplier/listByPageNo", {
        pageNum: 1,
        pageSize: 1000,
      });
      if (result.code == 200) {
    ...
    checkAttr(data) {
      if (data.type === 0) {
        ...
        data.isIndeterminate =
          data.attrValSelected.length !== data.attrValList.length;
        data.checkAll = !data.isIndeterminate;
        Vue.set(
          this.dataForm.attributeMap,
          data.id,
          data.attrValSelected.join(",")
        );
      } else if (data.type === 2) {
        //输入框
        Vue.set(this.dataForm.attributeMap, data.id, data.attrValSelected);
    ...
    async loadData(productId) {
      ...
      if (result.code == 200) {
        const dataForm = {
          ...
          firstCategoryNode: result.data.firstCategoryNode
            .split(",")
            .splice(1, 3),
        ...
        (this.oldMoneyForm.isCreditPoints = result.data.isCreditPoints + ""),
          ...
          (this.oldMoneyForm.pointDiscountPoint =
            result.data.pointDiscountPoint),
        ...
        //回显属性参数
        this.categorySelectionOnChange(
    ...
    save() {
      this.$refs.dataFormInfor.validate(async (valid) => {
        if (valid) {
          ...
          if (!flag) return;
          if (
            this.userObject.accountType == 9 &&
            this.oldMoneyForm.money == 1
          ) {
          ...
          if (this.oldMoneyForm.isCreditPoints == 1) {
            ...
            if (!patter.test(this.oldMoneyForm.registerPoint)) {
              this.$message.warning(
                "注册用户积分抵扣值为数字或者数字保留2位小数！"
              );
              return;
            }
          }
          if (
            this.oldMoneyForm.shop == "1" &&
            this.serviceForm.store == "1" &&
          ...
          if (this.dataForm.firstCategoryNode.length > 0) {
            firstCategoryNode =
              "2," + this.dataForm.firstCategoryNode.join(",");
          }
          ...
          let params = Object.assign(points, {
            ...
            productType: this.oldMoneyForm.shop,
          });
    ...
    beforeVideoUpload(file) {
      ...
      if (!isFile) {
        this.$message.error("上传文件只能是 mp4、flv 格式!");
      }
      if (!isLt30M) {
