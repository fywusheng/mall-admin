<template>
  <div class="main-content">
    ...
    <el-table class="custom-table" :data="dataList"
        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="退货凭证"
              v-if="props.row.img1||props.row.img2||props.row.img3||props.row.img4||props.row.img5">
            ...
            <el-form-item label="快递凭证"
              v-if="props.row.returnsType===2&&props.row.goodsState===2&&(props.row.expressImg1||props.row.expressImg2||props.row.expressImg3)">
              <el-image v-if="props.row.expressImg1" :src="props.row.expressImg1"
                style="width: 130px; height: 120px">
                <div slot="placeholder" class="image-slot">
                  加载中<span class="dot">...</span>
                </div>
              </el-image>
              <el-image v-if="props.row.expressImg2" :src="props.row.expressImg3"
                style="width: 130px; height: 120px">
                <div slot="placeholder" class="image-slot">
                  加载中<span class="dot">...</span>
                </div>
              </el-image>
              <el-image v-if="props.row.expressImg3" :src="props.row.expressImg3"
                style="width: 130px; height: 120px">
                <div slot="placeholder" class="image-slot">
                  加载中<span class="dot">...</span>
                </div>
      ...
      <el-table-column prop="returnsCode" label="售后编号" width="150px"
      ...
      <el-table-column prop="orderCode" label="归属订单" width="200px" align="center">
        <template slot-scope="scope">
          <span  style="color: #FF5500;"
            @click="forward2DetailsPage(scope.row)">{{scope.row.orderCode}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="" label="操作" width="290px" align="center">
        <template slot-scope="scope">
          ...
          <el-button size="mini" :disabled="Number(scope.row.returnsStatus)!=0"
            @click="receiving(scope.row)">商家收货</el-button>
          <el-button size="mini" :disabled="Number(scope.row.returnsStatus)!=1"
            @click="appoveRefund(scope.row)">同意退款</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    forward2DetailsPage(row) {
      this.$router.push({ name: 'Order-Detail', params: { orderCode: row.orderCode } })
    },
    async audit(row) {
