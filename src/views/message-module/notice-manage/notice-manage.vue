...
export default {
  ...
  methods: {
    ...
    async loadNoticeList() {
      ...
      try {
        const { type, data } = await post(path + "/notice/sys/selectNotice", this.searchData)
        if (type === "success") {
    ...
    async onChangeStatus(row) {
      ...
      this.listLoading = true
      try {
        const { type } = await post(path + "/notice/sys/updtNotcRunStas", { notcId: row.notcId, runStas: row.runStas ? 0 : 1 })
        if (type === "success") {
          this.listLoading = false
          this.$message({
            type: "success",
    ...
    async handleEditNotice() {
      ...
      try {
        const { type } = await post(path + "/notice/sys/addOrUpdateSysNoticeService", params)
        if (type === "success") {
    ...
    async handleDeleteNotice(id) {
      this.listLoading = true
      try {
        const { type } = await post(path + "/notice/sys/deleteNoticeService", { notcId: id })
        if (type === "success") {
          this.$message({
            type: "success",
            message: "删除成功!"
    ...
    async onClickSendNotice(row) {
      this.listLoading = true
      try {
        const { type } = await post(path + "/mock/api/notice/push", { id: row.id })
        if (type === "success") {
          this.$message({
            type: "success",
            message: "推送成功!"
