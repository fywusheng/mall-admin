  ...
  <div class="app-container">
    ...
    <div class="table-wrap">
      ...
      <el-table ref="table" v-loading="listLoading" height="460px" :data="list"
        ...
        <el-table-column label="角色名称" prop="roleName" show-overflow-tooltip align="center" />
        <el-table-column label="状态" min-width="120" width="220" align="center"
          show-overflow-tooltip>
          <template slot-scope="scope">
            <el-tag :type="scope.row.roleStas ? 'success' : 'danger'">
              {{ scope.row.roleStas ? "开启" : "关闭"}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="创建人" prop="crterName" show-overflow-tooltip align="center" />
        ...
        <el-table-column align="center" fixed="right" prop="created_at" label="操作" width="250">
          <template slot-scope="scope">
            ...
            <el-link
              :title="$store.getters.roleIdList.includes(scope.row.roleId) ? '不可设置自身拥有的角色权限' : ''"
              :disabled="$store.getters.roleIdList.includes(scope.row.roleId)" size="small"
              @click="handleMenuList(scope.row.roleId)">权限</el-link>
            <el-link
    ...
    <el-dialog :close-on-click-modal="false" :title="addFlag ? '新增' : '修改'"
      ...
      <el-form ref="formAdd" :model="formAdd" :rules="formAddRule" :label-width="formLabelWidth"
        size="small">
        <el-form-item label="角色名称" prop="roleName">
          <el-input v-model="formAdd.roleName" clearable :maxlength="50" autocomplete="off"
            placeholder="请输入角色名称" />
        </el-form-item>
        ...
        <el-form-item label="角色描述" prop="roleDesc">
          <el-input type="textarea" resize="none" :autosize="{minRows: 5, maxRows: 8}"
            v-model="formAdd.roleDesc"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog :close-on-click-modal="false" title="角色权限" :visible.sync="dialogFormPreviewVisible"
      width="600px">
      <div class="btn-box">
        <el-button type="primary" size="small" @click="foldOrCollapse">
          {{!isExpand ? "全部展开" : "全部折叠" }}</el-button>
        <el-button type="danger" size="small" @click="selectAllOrReversal">
          {{!isSelectAll ? "全部选中" : "全部不选" }}</el-button>
        <el-button type="warning" size="small" @click="toggleSelect">全部反选</el-button>
      </div>
      <y-tree ref="tree" show-checkbox node-key="menuId" @expandChange="expandChange"
        @check-change="checkChange" :settings="defaultProps" class="tree-warp" :data="menuList">
      </y-tree>
      <div slot="footer" class="dialog-footer">
