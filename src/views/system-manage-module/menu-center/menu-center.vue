<template>
  <div class="page-menu app-container">
    <div class="content">
      <div class="left">
        <y-com-tree ref="tree" :render-content="renderContent" :data="menuList" node-key="menuId"
          :expand-on-click-node="false" default-expand-all :props="defaultProps"
        ...
        <div class="table-wrap">
          <div class="table-edit-box clearfix">
            <div class="table-btn-right">
              <el-button v-show="formSearch.menuId !== '0'" type="primary" size="mini"
                @click="searchRoot()">一级菜单</el-button>
              <el-button class="two-words" type="default" icon="el-icon-circle-plus-outline" size="mini" @click="addOrEdit()">添加
              </el-button>
            </div>
          </div>
          <el-table ref="table" v-loading="listLoading" height="460px" :data="list"
            ...
            <el-table-column label="菜单名称" prop="title" show-overflow-tooltip align="center" />
            <el-table-column label="菜单URL" prop="path" min-width="120" align="center"
              show-overflow-tooltip />
            <el-table-column label="菜单图标" show-overflow-tooltip width="80" align="center">
              <template slot-scope="scope">
                <i v-if="scope.row.icon" :class="scope.row.icon" class="table-icon" />
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column label="状态" show-overflow-tooltip width="100" align="center">
              <template slot-scope="scope">
                <el-tag :type="scope.row.status ? 'success' : 'danger'">
                  {{scope.row.status ? "已启用" : "已停用"}}</el-tag>
              </template>
            </el-table-column>
    ...
    <el-dialog :close-on-click-modal="false" :title="addFlag?'新增':'修改'"
      :visible.sync="dialogFormVisible" width="600px" class="add-dialog">
      <el-form ref="formAdd" :model="formAdd" :rules="formAddRule" label-width="130px" size="small">
        <el-form-item label="所属父菜单" prop="parentMenuId">
          <y-cascader :key="formAdd.menuId" :delValue="formAdd.menuId"
            :settings="{'value':'menuId','label':'title'}" :data="selectmenuList" clearable
            v-model="formAdd.parentMenuId" />
        </el-form-item>
        ...
        <el-form-item label="重定向菜单URL" v-show="isRedirect"
          :rules="isRedirect ? { required: true, message: '请选择重定向地址', trigger: 'change' } : []"
          prop="redirect">
          <el-select v-model="formAdd.redirect" filterable :maxlength="50" clearable
            placeholder="请选择重定向菜单URL">
        ...
        <el-form-item label="菜单图标" prop="icon">
          <div class="clearfix">
            ...
            <el-button class="fl btn-choose" type="primary" icon="el-icon-edit" size="small"
              @click="handleAddIcon">{{ formAdd.icon?'重新选择':'添加图标' }}</el-button>
          </div>
        </el-form-item>
        ...
        <el-form-item label="菜单组件标识" prop="name">
          <el-select v-model="formAdd.name" filterable :maxlength="50" clearable
            placeholder="请选择菜单组件标识">
            <el-option :key="item.value" v-for="item in menuComponentList" :value="item.value"
    ...
    <!-- 图标弹窗 -->
    <el-dialog :close-on-click-modal="false" title="选择图标" :visible.sync="dialogIconVisible"
      width="600px" height="700px" class="icon-dialog clearfix">
      <div class="clearfix icon-wrap">
        <div v-for="(item,index) of elementIcons" :key="item" class="icon-item"
          :class="selectIconIndex==index?'active':''" @click="handleSelectIcon(item,index)">
          <i :class="'el-icon-' + item" />
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="small" @click="handleSelect">选 择</el-button>
        <el-button size="small" @click="()=>{selectIconIndex = '-1';dialogIconVisible=false}">取 消
        </el-button>
      </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    filterData2(arr) {
      ...
      return arr.filter(item => {
        if (item.children) {
          item.children = this.filterData2(item.children)
        }
