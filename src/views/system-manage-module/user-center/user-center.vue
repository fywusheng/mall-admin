  ...
  <div class="app-container">
    <el-form ref="formSearch" :model="formSearch" :inline="true" class="search-form clearfix"
      ...
      <el-form-item class="fr">
        ...
        <el-button class="two-words" icon="el-icon-search" type="primary" size="mini" @click="handleSearch">查询
        </el-button>
        <el-button type="default" icon="el-icon-circle-plus-outline" size="mini" @click="addOrEdit()">添加</el-button>
      </el-form-item>
    </el-form>
    <div class="table-wrap">
      ...
      <el-table ref="table" v-loading="listLoading" :data="list" height="450px"
        element-loading-text="加载中..." fit highlight-current-row>
        <el-table-column align="center" type="index" label="序号" width="50">
          <template
            slot-scope="scope">{{ (scope.$index + 1 )+ (formSearch.pageNum - 1) * formSearch.pageSize }}</template>
        </el-table-column>
        <el-table-column align="center" label="姓名" prop="name" min-width="150" />
        <el-table-column align="center" label="手机号" prop="acct" min-width="150" />
        <el-table-column align="center" label="创建人" prop="crteAcct" min-width="150" />
        ...
        <el-table-column align="center" label="最后登录时间" prop="crteTime" min-width="150">
          <template slot-scope="scope">
    ...
    <el-dialog :close-on-click-modal="false" :title="addFlag ? '新增' : '修改'"
      ...
      <el-form ref="formAdd" :validate-on-rule-change="true" :model="formAdd" :rules="formAddRule"
        :label-width="formLabelWidth" size="small">
        <el-form-item label="姓名" prop="name">
          <el-input v-model="formAdd.name" clearable autocomplete="off" placeholder="请输入姓名"
            maxlength="11" />
        </el-form-item>
        <el-form-item label="手机号" prop="acct">
          <el-input v-model="formAdd.acct" clearable autocomplete="off" placeholder="请输入手机号"
            maxlength="11" />
        </el-form-item>
        ...
        <el-form-item label="角色" prop="roles" v-show="formAdd.acctId !== '1'">
          <el-select :disabled="formAdd.acctId === $store.getters.id" v-model="formAdd.roles"
            filterable clearable placeholder="请选择角色" collapse-tags multiple size="small">
        ...
        <el-form-item label="密码" v-show="addFlag || isEditPwd"
          :rules="isEditPwd ? [{ required: true, validator: validatePass, trigger: 'blur' }] : {}"
          prop="pwd">
          <el-input v-model="formAdd.pwd" clearable type="password" autocomplete="off"
            placeholder="请输入8-20包含数字、字母、特殊符号任意两种组合" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="()=>{dialogFormVisible=false; this.listLoading = false;}">取
          消</el-button>
