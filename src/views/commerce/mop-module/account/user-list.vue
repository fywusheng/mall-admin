<template>
  <div class="main-content" style="padding-top:0px;padding-bottom:0px">
    <el-form :inline="true">
      <el-form-item label="">
        <el-input v-model="searchParams.accountCode" placeholder="请输入账号名称..." clearable
          size="mini"></el-input>
      </el-form-item>
      <el-form-item label="">
        <el-select v-model="searchParams.merchantId" size="mini" collapse-tags filterable clearable
          placeholder="请选择所属商户...">
          <el-option v-for="item in merchantListOptions" :key="item.id" :label="item.name"
    ...
    <el-table class="custom-table" :data="dataList"
      ...
      <el-table-column prop="isPrimary" label="是否主账号" :formatter="formatIsPrimary" width="100px"
      ...
      <el-table-column prop="" label="操作" align="center" width="150px">
        <template slot-scope="scope">
          <el-link icon="el-icon-view" :underline="false"
            @click="authorization(scope.row)">授权&nbsp;&nbsp;</el-link>
        </template>
      </el-table-column>
    </el-table>
