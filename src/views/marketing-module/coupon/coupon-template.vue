<template>
  <div class="div-layout">
    ...
    <el-form class="data-form" :model="dataForm" :rules="dataRules" :v-loading="loading"
      ref="dataForm" label-width="150px" label-position="top" size="mini">
        ...
        <tr>
          ...
          <td width="40%">
            <el-form-item label="优惠券名称" prop="name">
              <el-input v-model="dataForm.name" :disabled="dataForm.approveState==1"
                placeholder="请输入优惠券名称..." maxlength="64" style="width:80%"></el-input>
            </el-form-item>
          </td>
          <td width="50%">
        ...
        <tr>
          ...
          <td width="45%">
            <el-form-item label="优惠券面额" prop="denomination">
              <el-input v-model="dataForm.denomination" :disabled="dataForm.approveState==1"
                placeholder="请输入优惠券面额..." maxlength="5" oninput="value=value.replace(/[^\d.]/g,'')"
                style="width:80%">
          ...
          <td width="50%">
            <el-form-item label="发放数量" prop="num">
              <el-input v-model="dataForm.num" :disabled="dataForm.approveState==1"
                placeholder="请输入优惠券发放数量..." maxlength="60" style="width:80%"></el-input>
            </el-form-item>
          </td>
        </tr>
        <tr>
          ...
          <td width="45%">
            <el-form-item label="使用门槛" prop="checkThreshold">
              <template>
                <el-radio-group v-model="dataForm.checkThreshold"
                  :disabled="dataForm.approveState==1">
                  <el-radio :label="0" style="display:block;height:40px">无门槛</el-radio>
                  <el-radio :label="1" style="display:block;height:50px">订单满：
                    <span style="margin-top:0px">
                      <el-input :disabled="dataForm.checkThreshold==0"
                        v-model="dataForm.thresholdValue" maxlength="6" style="width:80%"><template
                          slot="append">元可用</template></el-input>
                    </span>
                  </el-radio>
                </el-radio-group>
              </template>
          ...
          <td width="50%">
            <el-form-item label="优惠券规则" prop="description">
              <el-input v-model="dataForm.description" :disabled="dataForm.approveState==1"
                placeholder="请输入优惠券使用规则..." maxlength="256" type="textarea" rows="3"
        ...
        <tr>
          ...
          <td width="45%">
            <el-form-item label="发放方式" prop="gettingMethod">
              <template>
                <el-radio-group v-model="dataForm.gettingMethod"
                  :disabled="dataForm.approveState==1">
      ...
      <table width="100%">
        <tr>
          ...
          <td width="45%">
            <el-form-item label="" prop="goodsScope">
              <template>
                <el-radio-group v-model="dataForm.goodsScope" :disabled="dataForm.approveState==1"
                  @change="goodsScopeOnchange">
        ...
        <tr v-if="dataForm.goodsScope==1||dataForm.goodsScope==2">
          <td width="5%"></td>
          <td colspan="2">
            <el-table class="custom-table" :data="currentRecords"
              :header-cell-style="{background:'#FAF9F7',color:'#000000'}" size="mini"
              v-loading="loading" style="width:100%">
              <div slot="empty" class="empty-wrap">
                <i class="iconfont icon-tishi"></i><span>系统暂无数据</span>
              </div>
              <el-table-column type="index" label="序号" width="45px" align="center">
              </el-table-column>
              ...
              <el-table-column prop="id" label="商品ID" align="center" width="180px">
              </el-table-column>
              <el-table-column prop="name" label="商品名称"> </el-table-column>
              <el-table-column prop="code" label="商品编码" align="center" width="180px">
              </el-table-column>
              <el-table-column align="center" width="350px">
                <template slot="header">
                  <!-- <el-input v-model="product.id" size="mini" style="width:150px" placeholder="输入商品ID搜索"/>
                <el-button type="primary" icon="el-icon-search" @click="loadData" size="mini">查询</el-button> -->
                  <el-button type="default" icon="el-icon-circle-plus-outline"
                    @click="showGoodsList()" :disabled="dataForm.approveState==1" size="mini">导入
                  </el-button>
                </template>
                <template slot-scope="scope">
                  <el-link icon="el-icon-view" :underline="false" @click="removeProduct(scope.row)">
                    移除</el-link>
                </template>
              </el-table-column>
            </el-table>
            <el-pagination background v-show="totalCount > 10" @size-change="changeSize"
              ...
              layout="total, slot, jumper, prev, pager, next" :total="totalCount" style="width:95%">
            </el-pagination>
          </td>
        </tr>
      </table>
    ...
    <el-row type="flex" style="height:100px;text-align:center">
      <el-col :span="22">
        ...
        <el-button type="primary" :loading="sending" :disabled="dataForm.approveState==1||sending"
          @click="save">
          {{sending ?
          '正在保存...':'提交'}}
        </el-button>
