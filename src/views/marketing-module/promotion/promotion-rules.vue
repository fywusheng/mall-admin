<template>
  <div class="div-layout">
    ...
    >
        ...
        <tr>
          ...
          <td width="50%">
            <el-form-item label="发放方式" prop="gettingMethod">
              <template>
                <el-radio-group
                  v-model="dataForm.gettingMethod"
                  :disabled="dataForm.auditState == 1"
                  @input="handleGettingChange"
                >
        ...
        <table width="100%">
          ...
          <tr>
            ...
            <td width="90%">
              ...
              <div>
                <el-table
                  v-if="dataForm.activityCoupons"
                  ref="couponList"
                  :data="dataForm.activityCoupons"
        ...
        <tr>
            ...
            <el-form-item label="适用会员:" prop="usersScope">
              <template>
                <el-radio-group v-model="dataForm.usersScope">
                  <el-radio
                    :label="0"
                    :disabled="dataForm.gettingMethod == 0"
          ...
          <td width="65%" style="align: left">
            <template v-if="dataForm.usersScope === 1">
              <el-link type="primary" @click="addUsers(1)"
            ...
            <template v-if="dataForm.usersScope === 2">
              <el-link type="primary" @click="addUsers(2)"
                ><i class="el-icon-circle-plus"
                  >&nbsp;&nbsp;导入禁用会员列表</i
                ></el-link
              >
      ...
      <coupon-edit-template
        ref="coupontemplate"
        v-on:addCouponList="addCouponList"
      >
      </coupon-edit-template>
...
export default {
    ...
    return {
      ...
      dataRules: {
        ...
        activityCoupons: [
          {
            required: true,
            validator: validatorActivityCoupons,
            trigger: "blur",
          },
        ],
        usersScope: [
  ...
  methods: {
    ...
    handleGettingChange(v) {
      if (v == "0") {
        this.dataForm.usersScope = "0";
      }
    },
    ...
    async loadData() {
      const result = await fetch("/marketing/promotion/activity/loading", {
      ...
      this.$refs.dataForm.validate(async (valid) => {
        if (valid) {
          ...
          const result = await post(
            "/marketing/promotion/activity/update",
            params
          );
          this.sending = false;
          if (result.code == 200) {
    ...
    addCoupons() {
      this.$refs.coupontemplate.show(
        true,
        this.$route.params.id,
        this.dataForm.activityCoupons
      );
    },
